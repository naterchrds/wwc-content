 ...
 * @since  0.0.4
 */
class cmb_Meta_Box_PPS_Sanitize {

	/**
	 * A CMB field object
	 * @var cmb_Meta_Box_PPS_field object
	 */
	public $field;
	...
	public function text_datetime_timestamp_timezone( $value, $repeat = false ) {
		...
		if ( empty( $tzstring ) )
			$tzstring = cmb_Meta_Box_PPS::timezone_string();

		$offset = cmb_Meta_Box_PPS::timezone_offset( $tzstring, true );
	...
	public function _save_file_id( $value ) {
		...
		$field      = new cmb_Meta_Box_PPS_field( $args, $group );
		$id_key     = $field->_id();
		...
		if ( $value && ! $id_val ) {
			$id_val = cmb_Meta_Box_PPS::image_id_from_url( $value );
		}
