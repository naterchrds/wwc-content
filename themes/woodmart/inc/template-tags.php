...
if( ! function_exists( 'woodmart_extra_footer_action' ) ) {
	function woodmart_extra_footer_action() {
		if (woodmart_needs_footer()) do_action( 'woodmart_after_footer' );
	}
...
if( ! function_exists( 'woodmart_header_block_search' ) ) {
	function woodmart_header_block_search() {
		...
		?>
			<div class="search-button woodmart-search-<?php echo esc_attr( $header_search ); ?>">
				<a href="#">
		...
		?>
			<form role="search" method="get" id="searchform" class="searchform <?php echo esc_attr( $class ); ?>" action="<?php echo esc_url( home_url( '/' ) ); ?>" <?php echo ( $data ); ?>>
				<div>
					...
					<button type="submit" id="searchsubmit"><?php echo esc_attr_x( 'Search', 'submit button', 'woodmart' ); ?></button>
				</div>
			</form>
...
if( ! function_exists( 'woodmart_header_block_main_nav' ) ) {
	function woodmart_header_block_main_nav() {
		...
		?>
			<div class="main-nav site-navigation woodmart-navigation <?php echo esc_attr( $classes ); ?>" role="navigation">
				<?php 
	...
	function woodmart_header_block_mobile_icon() {
		?>
			<div class="mobile-nav-icon">
				...
				<span class="mobile-menu-btn-label"><?php esc_html_e('Menu', 'woodmart'); ?></span>
			...
			<div class="woodmart-header-links">
				<ul>
					<?php foreach ($links as $key => $link): ?>
						<li class="item-level-0 menu-item-<?php echo esc_attr( $key ); ?>"><a href="<?php echo esc_url( $link['url'] ); ?>"><span><?php echo esc_attr( $link['label'] ); ?></span></a>
				...
				$links['register']['dropdown'] = '
					<div class="sub-menu-dropdown color-scheme-' . $color_scheme . '">
						<div class="login-dropdown-inner">
							<h3 class="login-title"><span>' . esc_html__('Sign in', 'woodmart') . '</span><a href="' . esc_url( $account_link ) . '">' . esc_html__('Create an Account', 'woodmart') . '</a>' . '</h3>
...
if( ! function_exists( 'woodmart_topbar_links' ) ) {
	...
	function woodmart_topbar_links ( $items = '', $args = array(), $return = false ) {
	    if ( 
	    	...
	    	if( woodmart_get_opt('header_wishlist') == 'topbar' ) {
			...
			if( ! empty( $links ) && ( woodmart_get_opt( 'topbar_links' ) ||  $args->theme_location == 'mobile-menu' ) ){
				foreach ($links as $key => $link) {
					$class = '';
					$items .= '<li class="menu-item item-level-0 ' . $class . ' menu-item-' . $key . '">';
			...
			<form method="post" class="login <?php if ( $hidden ) echo 'hidden-form'; ?>" <?php echo ( ! empty( $action ) ) ? 'action="' . esc_url( $action ) . '"' : ''; ?> <?php if ( $hidden ) echo 'style="display:none;"'; ?>>
				...
				<div class="login-form-footer">
					...
					<label for="rememberme" class="remember-me-label inline">
						<input name="rememberme" type="checkbox" id="rememberme" value="forever" /> <?php esc_html_e( 'Remember me', 'woodmart' ); ?>
					</label>
				</div>
